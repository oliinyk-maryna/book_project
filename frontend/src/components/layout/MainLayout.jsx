...
import { Home, Search, BookOpen, Users, BarChart2, Lock } from 'lucide-react';
...
export default function MainLayout({ children, user, onOpenAuth }) {
  ...
  // Назви адаптовано під ваш скріншот
  const NAV_ITEMS = [
    { label: 'Головна', icon: Home, path: '/' },
    { label: 'Каталог', icon: Search, path: '/discover' },
    { label: 'Моя полиця', icon: BookOpen, path: '/library', auth: true },
    { label: 'Спільноти', icon: Users, path: '/clubs', auth: true },
    { label: 'Статистика', icon: BarChart2, path: '/analytics', auth: true },
  ];
  ...
  return (
    <div className="min-h-screen bg-[#FDFDFD] flex flex-col md:flex-row font-sans text-stone-900">
      ...
      <aside className="hidden md:flex flex-col w-60 fixed inset-y-0 left-0 bg-white border-r border-stone-200 z-40">
        <div className="p-8">
          <Link to="/" className="font-serif font-black text-3xl text-stone-900 tracking-tight">ReadLounge.</Link>
        </div>
        <nav className="flex-1 px-4 space-y-1.5">
          {NAV_ITEMS.map(item => {
            const isLocked = item.auth && !user;

            // Якщо пункт потребує авторизації, а користувач не увійшов
            if (isLocked) {
              return (
                <button
                  key={item.path}
                  onClick={() => onOpenAuth('login')}
                  className="w-full flex items-center justify-between px-5 py-3.5 rounded-2xl transition-all text-[#A19D94] hover:bg-stone-50"
                >
      ...
      <div className="flex-1 md:ml-60 flex flex-col min-h-screen relative">
        <header className="sticky top-0 z-30 bg-white/90 backdrop-blur-xl border-b border-stone-200 h-20 flex items-center justify-between px-6 md:px-10">
          <div className="md:hidden font-serif font-black text-2xl text-stone-900">ReadLounge.</div>
          ...
          <div className="flex items-center gap-4 shrink-0 ml-4">
            ...
            {user ? (
              <button onClick={() => navigate('/profile')} className="w-10 h-10 rounded-full bg-stone-900 text-white font-bold flex items-center justify-center overflow-hidden shadow-sm hover:scale-105 transition-transform border border-stone-200">
                {user.avatar_url ? <img src={user.avatar_url} className="w-full h-full object-cover" alt="Avatar" /> : user.username[0].toUpperCase()}
              </button>
            ) : (
        ...
        <div className="flex items-center justify-around h-16 px-2 overflow-x-auto no-scrollbar">
          {NAV_ITEMS.map(item => {
            ...
            return (
              <button 
                key={item.path} 
                onClick={() => isLocked ? onOpenAuth('login') : navigate(item.path)}
                className={`flex flex-col items-center justify-center gap-1 min-w-[64px] h-full px-1 ${
                  location.pathname === item.path ? 'text-stone-900' : 'text-stone-400'
                }`}
              >
                <div className="relative">
                  <item.icon className="w-5 h-5" />
                  {/* Маленький замочок для мобільної версії */}
                  {isLocked && <Lock className="w-3 h-3 absolute -bottom-1 -right-1 bg-white text-stone-400 rounded-full" />}
                </div>
